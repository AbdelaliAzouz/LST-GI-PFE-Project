import './Etablissements.css'
import Navbar2 from '../../../Navbar/Navbar2/Navbar2'
import LocationOnIcon from '@material-ui/icons/LocationOn';

// Pop-up
import Popup from 'reactjs-popup';
import 'reactjs-popup/dist/index.css';
import { Avatar } from "@material-ui/core";

const Etablissements = () => {
    
    return (
    <div>
        <Navbar2/>
        <div className="labo">
            <h1 className="header-labo-equipe">
                Etablissements
            </h1>
            <h2 className="header-labo-equipe">
                Univesité Abdelmalek Essaidi
            </h2>
            
            <div className="liste-labos-container">
                <div className="liste-labos">
                <div className="Location">
                


                        <LocationOnIcon/>
                        <h6>Tanger</h6>
                        </div>

                        <Popup
                        className="popupstyle" trigger={<button className="btn-labo-equipe" ><div className="display"><Avatar src="data:image/jpeg;base64," alt="Logo Etablissement"/><p>Faculté des Sciences et Techniques</p></div></button>}
                        position="right center"
                        >
                            
                            <div className="popup-things">
                                <p>Site oficielle : <br /> <a className="site-oficielle" href="http://www.fstt.ac.ma/Portail/index.php" target="_blank">www.fstt.ac.ma</a></p>
                                <p>
                                  Cosulter les : 
                                </p>
                                <p><a className="a-labo" href="http://localhost:3000/Labos" target="_blank">Les laboratoire de l'établissement</a></p>
                                <p><a className="a-chercheurs" href="" target="_blank">Les chercheurs de l'établissement</a></p>
                            </div>
                        </Popup>
                    
                    <Popup
                        className="popupstyle" trigger={<button className="btn-labo-equipe"><div className="display"><Avatar src="data:image/jpeg;base64," alt=""/><p>École nationale des sciences appliquées</p></div></button>}
                        position="right center"
                        >
                            <div className="popup-things">
                                <p>Site oficielle : <br /> <a className="site-oficielle" href="http://ensat.ac.ma/Portail/" target="_blank">www.ensat.ac.ma</a></p>
                                <p>
                                  Cosulter les : 
                                </p>
                                <p><a className="a-labo" href="" target="_blank">Les laboratoire de l'établissement</a></p>
                                <p><a className="a-chercheurs" href="" target="_blank">Les chercheurs de l'établissement</a></p>
                            </div>
                    </Popup>
               
                    <Popup
                        className="popupstyle" trigger={<button className="btn-labo-equipe"><div className="display"><Avatar src="https://www.uae.ma/website/sites/default/files/styles/actualit_s/public/logo_encgt_actualites.jpg?itok=IM78PpJ-" alt=""/><p>École Nationale de Commerce et de Gestion</p></div></button>}
                        position="left center"
                        >
                            <div className="popup-things">
                                <p>Site oficielle : <br /> <a className="site-oficielle" href="https://encgt.ma/" target="_blank">www.encgt.ma</a></p>
                                <p>
                                  Cosulter les : 
                                </p>
                                <p><a className="a-labo" href="" target="_blank">Les laboratoire de l'établissement</a></p>
                                <p><a className="a-chercheurs" href="" target="_blank">Les chercheurs de l'établissement</a> </p>
                            </div>
                    </Popup>
                </div>
                <div className="liste-labos">
                    <Popup
                        className="popupstyle" trigger={<button className="btn-labo-equipe"><div className="display"><Avatar src="https://www.guide-metiers.ma/wp-content/uploads/2019/03/fsjes-tanger.png" alt=""/><p>Faculté des Sciences Juridiques, Economiques et Sociales</p></div></button>}
                        position="right center"
                        >
                            <div className="popup-things">
                                <p>Site oficielle : <br /> <a className="site-oficielle" href="http://fdtanger.ma/portail/index.php" target="_blank">www.fdtanger.ma/portail</a></p>
                                <p>
                                  Cosulter les : 
                                </p>
                                <p><a className="a-labo" href="" target="_blank">Les laboratoire de l'établissement</a></p>
                                <p><a className="a-chercheurs" href="" target="_blank">Les chercheurs de l'établissement</a></p>
                            </div>
                    </Popup>
                 
                    <Popup
                        className="popupstyle" trigger={<button className="btn-labo-equipe"><div className="display"><Avatar src="https://www.uae.ma/website/sites/default/files/fmpt.jpg" alt=""/><p>Faculté de Médecine et de Pharmacie</p></div></button>}
                        position="right center"
                        >
                            <div className="popup-things">
                                <p>Site oficielle : <br /><a className="site-oficielle" href="http://www.fmpt.ac.ma/" target="_blank">www.fmpt.ac.ma</a></p>
                                <p>
                                   Cosulter les : 
                                </p>
                                <p><a className="a-labo" href="" target="_blank">Les laboratoire de l'établissement</a></p>
                                <p><a className="a-chercheurs" href="" target="_blank">Les chercheurs de l'établissement</a> </p>
                            </div>
                    </Popup>
                    <Popup
                         trigger={<button className="btn-labo-equipe-hide" ></button>}
                        >
                    </Popup>
                </div>
               


                <div className="liste-labos">
                <div className="Location">
                        <LocationOnIcon/>
                        <h6>Tetouan</h6>
                        </div>
                    <Popup
                        className="popupstyle" trigger={<button className="btn-labo-equipe"><div className="display"><Avatar src="https://cdn-07.9rayti.com/rsrc/cache/widen_224/uploads/2013/11/logo-fsjes-tetouan.jpg" alt=""/><p>Faculté des Sciences</p></div></button>}
                        position="right center"
                        >
                            <div className="popup-things">
                                <p>Site oficielle : <br /> <a className="site-oficielle" href="http://www.fst.ac.ma/site/" target="_blank">www.fst.ac.ma</a></p>
                                <p>
                                   Cosulter les : 
                                </p>
                                <p><a className="a-labo" href="" target="_blank">Les laboratoire de l'établissement</a></p>
                                <p><a className="a-chercheurs" href="" target="_blank">Les chercheurs de l'établissement</a></p>
                            </div>
                    </Popup>
                    <Popup
                        className="popupstyle" trigger={<button className="btn-labo-equipe"><div className="display"><Avatar src="https://upload.wikimedia.org/wikipedia/commons/thumb/e/e5/Ecole_Nationale_des_sciences_appliqu%C3%A9es_de_T%C3%A9touan.jpg/520px-Ecole_Nationale_des_sciences_appliqu%C3%A9es_de_T%C3%A9touan.jpg" alt=""/><p>Ecoles Nationales des Sciences Appliquées</p></div></button>}
                        position="right center"
                        >
                            <div className="popup-things">
                                <p>Site oficielle : <br /> <a className="site-oficielle" href="https://ensa-tetouan.ac.ma/" target="_blank">www.ensa-tetouan.ac.ma</a></p>
                                <p>
                                    Cosulter les : 
                                </p>
                                <p><a className="a-labo" href="" target="_blank">Les laboratoire de l'établissement</a></p>
                                <p><a className="a-chercheurs" href="" target="_blank">Les chercheurs de l'établissement</a></p>
                            </div>
                    </Popup>

                    <Popup
                        className="popupstyle" trigger={<button className="btn-labo-equipe"><div className="display"><Avatar src="http://www.fsjeste.ma/wp-content/uploads/2020/05/default-to-use-01.png" alt=""/><p>Faculté des Sciences Juridiques, Economiques et Sociales</p></div></button>}
                        position="left center"
                        >
                            <div className="popup-things">
                                <p>Site oficielle : <br /> <a className="site-oficielle" href="http://www.fsjeste.ma/" target="_blank">www.fsjeste.ma</a></p>
                                <p>
                                Cosulter les : 
                                </p>
                                <p><a className="a-labo" href="" target="_blank">Les laboratoire de l'établissement</a></p>
                                <p><a className="a-chercheurs" href="" target="_blank">Les chercheurs de l'établissement</a></p>
                            </div>
                    </Popup>
                </div>
             
                <div className="liste-labos">
                    <Popup
                        className="popupstyle" trigger={<button className="btn-labo-equipe"><div className="display"><Avatar src="https://cdn-07.9rayti.com/rsrc/cache/widen_292/uploads/2014/09/logo-ens-tetouan.jpg" alt=""/><p>Ecole Normale Supérieure</p></div></button>}
                        position="right center"
                        >
                            <div className="popup-things">
                                <p>Site oficielle : <br /> <a className="site-oficielle" href="http://www.enstetouan.ma/" target="_blank">www.enstetouan.ma</a></p>
                            
                                <p><a className="a-labo" href="" target="_blank">Les laboratoire de l'établissement</a></p>
                                <p><a className="a-chercheurs" href="" target="_blank">Les chercheurs de l'établissement</a></p>
                            </div>
                    </Popup>
                    <Popup
                         trigger={<button className="btn-labo-equipe-hide" ></button>}
                        >
                    </Popup><Popup
                         trigger={<button className="btn-labo-equipe-hide" ></button>}
                        >
                    </Popup>
               
                </div>
               
                <div className="liste-labos">
                <div className="Location">
                        <LocationOnIcon/>
                        <h6>Larache</h6>
                        </div>
                    <Popup
                        className="popupstyle" trigger={<button className="btn-labo-equipe"><div className="display"><Avatar src="https://www.fpl.ac.ma/site/wp-content/uploads/2020/01/logo-TR-2-2.png" alt=""/><p>Faculté Polydisciplinaire</p></div></button>}
                        position="right center"
                        >
                            <div className="popup-things">
                                <p>Site oficielle : <br /> <a className="site-oficielle" href="https://encgt.ma/" target="_blank">www.encgt.ma</a></p>
                                <p>
                                  Cosulter les : 
                                </p>
                                <p><a className="a-labo" href="" target="_blank">Les laboratoire de l'établissement</a></p>
                                <p><a className="a-chercheurs" href="" target="_blank">Les chercheurs de l'établissement</a></p>
                            </div>
                    </Popup>
                    <Popup
                         trigger={<button className="btn-labo-equipe-hide" ></button>}
                        >
                    </Popup><Popup
                         trigger={<button className="btn-labo-equipe-hide" ></button>}
                        >
                    </Popup>
        
                </div>
                <div className="liste-labos">
                <div className="Location">
                        <LocationOnIcon/>
                        <h6>Al hoceima</h6>
                        </div>
                    <Popup
                        className="popupstyle" trigger={<button className="btn-labo-equipe"><div className="display"><Avatar src="https://www.uae.ma/website/sites/default/files/fsth_logo_mini.jpg" alt=""/><p>Faculté des Sciences et Techniques</p></div></button>}
                        position="right center"
                        >
                            <div className="popup-things">
                                <p>Site oficielle : <br /> <a className="site-oficielle" href="https://encgt.ma/" target="_blank">www.encgt.ma</a></p>
                                <p>
                                   Cosulter les : 
                                </p>
                                <p><a className="a-labo" href="" target="_blank">Les laboratoire de l'établissement</a></p>
                                <p><a className="a-chercheurs" href="" target="_blank">Les chercheurs de l'établissement</a> </p>
                            </div>
                    </Popup>
                  
                    <Popup
                        className="popupstyle" trigger={<button className="btn-labo-equipe"><div className="display"><Avatar src="https://www.uae.ma/website/sites/default/files/ensah_logo.jpg" alt=""/><p>Ecole Nationale des Sciences Appliquées</p></div></button>}
                        position="right center"
                        >
                            <div className="popup-things">
                                <p>Site oficielle : <br /><a className="site-oficielle" href="https://encgt.ma/" target="_blank">www.encgt.ma</a></p>
                                <p>
                                   Cosulter les : 
                                </p>
                                <p><a className="a-labo" href="" target="_blank">Les laboratoire de l'établissement</a></p>
                                <p><a className="a-chercheurs" href="" target="_blank">Les chercheurs de l'établissement</a></p>
                            </div>
                    </Popup>
                    <Popup
                         trigger={<button className="btn-labo-equipe-hide" ></button>}
                        >
                    </Popup>

                </div>
            </div>
    
        </div>
        </div>
    );
};

export default Etablissements;

